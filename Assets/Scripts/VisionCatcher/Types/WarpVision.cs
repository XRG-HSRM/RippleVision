...
public class WarpVision : VisionCatcher
{
    [SerializeField] private TunnelVisionType type;
    ...
    private void Update()
    {
        ...
        switch (type)
        {
            case (TunnelVisionType.NoChange):
                break;
            case (TunnelVisionType.LogarithmicIncrease):
                LogarithmicIncrease();
                break;
            case (TunnelVisionType.ExponentialIncrease):
                ExponentialIncrease();
                break;
            default: break;
        }
    ...
    private void ExponentialIncrease()
    {
        ...
        if (controller.currentEyeTrackingScript.isValid)
        {
            ...
            float effectiveAngle = adjustedAngle < 0f ? 0 : adjustedAngle;
            float angleNormalized = Mathf.Clamp01(effectiveAngle / (90f - angleThreshold));
            float expScaleFactor = Mathf.Pow(2f, angleNormalized * growth) - 1f;  // growth controls the steepness
